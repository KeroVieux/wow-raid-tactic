      ...
      if (performance && performance.timing) {
        ...
        if (t.connectEnd && t.connectStart) {
          if (t.connectEnd && t.secureConnectionStart) {
            info.ssl = `${t.connectEnd - t.connectStart}ms (${t.connectEnd
              - t.secureConnectionStart}ms)`
          } else {
        ...
        if (t.domComplete && t.domLoading) {
          if (t.domContentLoadedEventStart && t.domLoading) {
            info.domCompleteDomLoaded = `${t.domComplete
              - t.domLoading}ms (${t.domContentLoadedEventStart
              - t.domLoading}ms)`
          } else {
        ...
        if (t.navigationStart && t.loadEventEnd) {
          info.totalDOM = `${t.loadEventEnd
            - t.navigationStart}ms (${t.domComplete - t.navigationStart}ms)`
        }
      }
    }, 0)
    return info
