...
if (e2eXMLExist) {
  ...
  fs.writeFile(`${resultPath}/e2e.html`, parsedAndRenderedData.replace(/<head>/, '<head>\n<meta charset="utf-8">'), (err) => {
    if (err) {
      console.error(err)
    }
  })
...
if (unitXMLExist) {
  ...
  fs.writeFile(`${resultPath}/unit.html`, parsedAndRenderedData.replace(/<head>/, '<head>\n<meta charset="utf-8">'), (err) => {
    if (err) {
      console.error(err)
    }
  })
